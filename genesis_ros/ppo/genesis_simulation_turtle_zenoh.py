import argparse
from genesis_ros.ppo.turtle_env import TurtleEnv
...
def eval(
    ...
    urdf_path: str = "assets/urdf/DirectTurtle/robot_urdf.urdf",
    device: str = "gpu",
    ...
    current_command = torch.zeros(3, device=gs.device)
    ...
    def on_twist_command(sample):
        ...
        try:
            ...
            current_command[0] = float(linear.get('x', 0.0))
            current_command[1] = float(linear.get('y', 0.0))
            current_command[2] = float(angular.get('z', 0.0))

            current_command[0] = torch.clamp(
                current_command[0], env.command_cfg["lin_vel_x_range"][0], env.command_cfg["lin_vel_x_range"][1]
            )
            current_command[1] = torch.clamp(
                current_command[1], env.command_cfg["lin_vel_y_range"][0], env.command_cfg["lin_vel_y_range"][1]
            )
            current_command[2] = torch.clamp(
                current_command[2], env.command_cfg["ang_vel_range"][0], env.command_cfg["ang_vel_range"][1]
            )

            command_received = True
            print(
                f"Received twist command: lin_vel_x={current_command[0]:.2f}, "
...
def cli_entrypoint():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="turtle-walking")
    parser.add_argument("--ckpt", type=int, default=100)
    parser.add_argument(
        "--urdf_path",
        help="Path to the URDF file",
        type=str,
        default="assets/urdf/DirectTurtle/robot_urdf.urdf",
    )
    parser.add_argument(
        "-i",
        "--interface",

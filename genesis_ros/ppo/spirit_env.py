...
def _parse_actuated_joint_names_from_urdf(urdf_path: str) -> List[str]:
...
class SpiritEnv:
    def __init__(
        ...
        # Resolve URDF path and joints
        self.urdf_path = urdf_path or env_cfg.get("urdf_path", "assets/urdf/spirit40/spirit40_ros2.urdf")
        joint_names = env_cfg.get("joint_names")
        if not joint_names:
            joint_names = _parse_actuated_joint_names_from_urdf(self.urdf_path)
        self.env_cfg["joint_names"] = joint_names
        self.num_actions = len(joint_names)
        self.num_obs = 9 + 3 * self.num_actions
        self.num_commands = command_cfg["num_commands"]

        # Default joint angles
        default_joint_angles = env_cfg.get("default_joint_angles") or {name: 0.0 for name in joint_names}
        self.env_cfg["default_joint_angles"] = default_joint_angles
    ...
    def step(self, actions):
        ...
        exec_actions = self.last_actions if self.env_cfg.get("simulate_action_latency", True) else self.actions
